...
class ChromaEmbeddingPipelineTextOnly:
    ...
    def chunk_text(self, text: str, metadata: Dict[str, Any]) -> List[Tuple[str, Dict[str, Any]]]:
        ...
        while start < len(text):
            ...
            if end < len(text):
                ...
                # Only use boundary if it's reasonable (not too early in the chunk)
    ...
    def get_embedding(self, text: str) -> List[float]:
        ...
        try:
            ...
            # Add error handling - return empty vector or re-raise
            raise
    ...
    def generate_document_id(self, file_path: Path, metadata: Dict[str, Any]) -> str:
        ...
        Args:
            ...
            metadata: Metadata containing chunk information
        ...
        doc_id = f"{mission}_{source}_chunk_{chunk_index:04d}"
    ...
    def extract_mission_from_path(self, file_path: Path) -> str:
        ...
        if 'apollo11' in path_str or 'apollo_11' in path_str:
            return 'apollo_11'
        elif 'apollo13' in path_str or 'apollo_13' in path_str or 'as13' in path_str:
            return 'apollo_13'
    ...
    def scan_text_files_only(self, base_path: str) -> List[Path]:
        ...
        # Also scan for .txt files directly in the base path
        base_txt_files = list(base_path.glob('*.txt'))
        files_to_process.extend(base_txt_files)
        if base_txt_files:
            logger.info(f"Found {len(base_txt_files)} text files in base directory")
    ...
    def add_documents_to_collection(self, documents: List[Tuple[str, Dict[str, Any]]],
        ...
        # Handle replace mode - delete existing documents first
        if update_mode == 'replace':
            existing_docs = self.get_file_documents(file_path)
            if existing_docs:
                self.collection.delete(ids=existing_docs)
                logger.info(f"Deleted {len(existing_docs)} existing documents for replace mode")
        ...
        for i in range(0, len(documents), batch_size):
            ...
            for text, metadata in batch:
                ...
                # Add to batch for new documents
    ...
    def process_all_text_data(self, base_path: str, update_mode: str = 'skip') -> Dict[str, int]:
        ...
        for file_path in files:
            try:
                ...
                logger.info(f"  Chunks: {len(documents)}, Added: {file_stats['added']}, "
