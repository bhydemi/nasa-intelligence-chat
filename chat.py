...
def main():
    ...
    with st.sidebar:
        ...
        if not available_backends:
            ...
            st.info("Please run the embedding pipeline first:\n`python embedding_pipeline.py --openai-key YOUR_API_KEY`")
            st.stop()
        ...
        mission_filter = st.selectbox(
            ...
            format_func=lambda x: x.replace("_", " ").title() if x != "all" else "All Missions",
            help="Filter results to a specific mission"
        )
